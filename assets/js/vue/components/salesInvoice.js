                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    <td>{{ sl + 1 }}</td>
                                    <td>{{ product.Product_Name }} - {{ product.Company_Name }}</td>
                                    <td>{{ product.SaleDetails_TotalQuantity }}</td>
                                    <td>{{ product.Unit_Name }}</td>
                                    <td>{{ product.SaleDetails_Rate }}</td>
                                    <td align="right">{{ (product.SaleDetails_TotalQuantity * product.SaleDetails_Rate) | decimal }}</td>
                                    <td align="right">{{ product.Discount_amount }}</td>
                                    <td align="right">{{ (product.SaleDetails_TotalAmount - product.Discount_amount) | decimal }}</td>
                                </tr>
                            </tbody>
                            <tfoot>
                                <tr style="font-weight:bold;">
                                    <td colspan="2" style="font-weight:normal;">Total</td>
                                    <td style="text-align:center;">{{ cart.reduce((prev, curr) => {return prev + parseFloat(curr.SaleDetails_TotalQuantity)}, 0) }}</td>
                                    <td colspan="3"></td>
                                    <td style="text-align:right;">{{ cart.reduce((prev, curr) => {return prev + parseFloat(curr.Discount_amount)}, 0) | decimal }}</td>
