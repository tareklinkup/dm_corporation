...
class Model_Table extends CI_Model
{
    ...
    public function currentStock($clauses = '')
    {
        $stock = $this->db->query("
            select * from(
                ...
                left join tbl_productcompany proc on proc.Company_SlNo = p.Company_ID
    ...
    public function specialCustomerDue($clauses = "")
    {
        $branchId = $this->session->userdata('BRANCHid');
        ...
        $dueResult = $this->db->query("
            ...
            (select billAmount - invoicePaid )as saleInvoiceDue,
            ...
            (select (ifnull(sum(pm.PurchaseMaster_TotalAmount), 0.00) ) 
                from tbl_purchasemaster pm
                join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
                where pm.status = 'a'
                and s.Supplier_Code = c.Customer_Code
            ) as purchaseBill,
            
            (select ifnull(sum(pm.PurchaseMaster_PaidAmount), 0.00) 
                from tbl_purchasemaster pm
                join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
                where pm.status = 'a'
                and s.Supplier_Code = c.Customer_Code
            ...
            ( select purchaseBill - purchasePaid) as purchaseInvoiceDue,
            
            (select ifnull(sum(sp.SPayment_amount), 0.00) 
            ...
            (select ifnull(sum(pr.PurchaseReturn_ReturnAmount), 0.00) 
            ...
            (select (c.previous_due + saleInvoiceDue + supplierPaid + purReturned + customerPaid) - (purchaseInvoiceDue + cashReceived + returnedAmount + supplierReceived)) as dueAmount
