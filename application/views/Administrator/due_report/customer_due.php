...
<div class="row" id="customerDueList">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;">
		...
		<div class="form-group" style="display: none" v-bind:style="{display: searchType == 'special' ? '' : 'none'}">
			<label class="col-sm-2 control-label no-padding-right">Select Special Customer</label>
			<div class="col-sm-2">
				<v-select v-bind:options="specialCustomers" v-model="selectedSpecialCustomer" label="display_name" placeholder="Select customer"></v-select>
			</div>
		</div>
		...
		methods: {
			onChangeSearchType() {
				if (this.searchType == 'customer' && this.customers.length == 0) {
					this.getCustomers();
				} else if (this.searchType == 'special' && this.specialCustomers.length == 0) {
					this.getCustomers();
			...
			getCustomers() {
				axios.get('/get_customers').then(res => {
					this.customers = res.data.filter(item => item.is_supplier == 0)
					this.specialCustomers = res.data.filter(item => item.is_supplier == 1)
				})
			},
			getAreas() {
			...
			getDues() {
				...
				let specialCustomerId = this.selectedSpecialCustomer == null ? null : this.selectedSpecialCustomer.Customer_SlNo;
