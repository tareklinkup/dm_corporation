	...
	<div class="row" style="display:none;" v-bind:style="{ display: showPage ? '' : 'none' }">
		<div class="col-md-8 col-md-offset-2">
			...
			<div id="invoiceContent">
						...
						<table _a584de>
							...
							<tbody>
								<tr v-for="(product, sl) in cart">
									<td>{{ sl + 1 }}</td>
									<td>{{ product.Product_Code }} - {{ product.Product_Name }} - {{ product.Company_Name }}</td>
				...
				<div class="row">
					<div class="col-xs-12">
						<strong>In Word: </strong> {{ convertNumberToWords(purchaseReturn.PurchaseReturn_ReturnAmount) }}<br><br>
						<strong>Note: </strong>
						<p style="white-space: pre-line">{{ purchaseReturn.PurchaseReturn_Description }}</p>
					</div>
				</div>
			</div>
		</div>
	...
	new Vue({
		...
		data(){
			return {
				...
				purchaseReturn:{
					PurchaseReturn_SlNo: null,
					PurchaseMaster_InvoiceNo: null,
					Supplier_IDdNo: null,
					PurchaseReturn_ReturnDate: null,
					PurchaseReturn_ReturnAmount: null,
					PurchaseReturn_Description: null,
				},
				cart: [],
				style: null,
				companyProfile: null,
		...
		methods:{
			getPurchaseReturn(){
				this.showPage = false;
				axios.post('/get_purchase_returns', {id: this.id}).then(res=>{
					this.purchaseReturn = res.data.returns[0];
					...
					<body>
						...
						<div class="container" style="position:fixed;bottom:15px;width:100%;">
							...
							<div class="row" style="font-size:12px;">
								<div class="col-xs-6">
									Print Date: ${moment().format('DD-MM-YYYY h:mm a')}, Printed by: ${this.purchaseReturn.AddBy}
								</div>
