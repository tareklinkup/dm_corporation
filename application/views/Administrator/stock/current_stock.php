...
<div id="stock">
    <div class="row">
        <div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
            ...
            <div class="form-group" style="margin-top:10px;" v-if="selectedSearchType.value == 'company'">
                <div class="col-sm-2" style="margin-left:15px;">
    ...
    <div class="row" v-if="searchType != null" style="display:none"
        ...
        <div class="col-md-12" style="display:flex">
            <a style="margin:0px auto 0px 0px" href="" v-on:click.prevent="print"><i class="fa fa-print"></i> Print</a>
            <a style="margin:0px 0px 0px auto" href="" v-on:click.prevent="printWithoutRate"><i class="fa fa-print"></i>
                Print Without Rates</a>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="table-responsive" id="stockContent">
                <table class="table table-bordered print-table" v-if="searchType == 'current'" style="display:none"
                ...
                <table class="table table-bordered print-table" v-if="searchType != 'current' && searchType != null"
                    style="display:none;"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.purchased_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.purchase_returned_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.damaged_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.sold_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.sales_returned_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.transferred_to_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.transferred_from_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.current_quantity).toFixed(2)},0)}}
                            </td>
                            <td style="text-align:center; font-weight:bold;">
                                {{ stock.reduce((prev,curr) => {return +prev + +parseFloat(curr.Product_Purchase_Rate).toFixed(2)},0)}}
                            </td>
                        </tr>
...
new Vue({
    ...
    methods: {
        getStock() {
            ...
            if (this.searchType == 'company' && this.selectedCompany == null) {
                alert('Select a Company');
                return;
            } else if (this.searchType == 'company' && this.selectedCompany != null) {
                parameters.companyId = this.selectedCompany.Company_SlNo;
                this.selectionText = "Company: " + this.selectedCompany.Company_Name;
            }
        ...
        onChangeSearchType() {
            if (this.selectedSearchType.value == 'category' && this.categories.length == 0) {
                this.getCategories();
            } else if (this.selectedSearchType.value == 'company' && this.companies.length == 0) {
                this.getCompanies();
        ...
        async printWithoutRate() {
            ...
            let rows = reportWindow.document.querySelectorAll('.print-table tr');
            rows.forEach((row, sl) => {
                if ((rows.length - 1) != sl) {
