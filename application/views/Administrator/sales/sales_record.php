					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:right;">{{ (sale.saleDetails[0].SaleDetails_TotalAmount - sale.saleDetails[0].Discount_amount) | decimal}}</td>
							...
							<tr v-for="(product, sl) in sale.saleDetails.slice(1)">
								...
								<td style="text-align:right;">{{ product.SaleDetails_TotalAmount - sale.saleDetails[0].Discount_amount }}</td>
				...
				<template v-if="searchTypesForDetails.includes(searchType)" style="display:none;" v-bind:style="{display: searchTypesForDetails.includes(searchType) ? '' : 'none'}">
					...
					<table class="record-table" v-if="selectedProduct == null">
						...
						<tbody>
							<template v-for="sale in sales">
								<tr>
									<td colspan="4" style="text-align:center;background: #ccc;">{{ searchType == 'category' ? sale.category_name : sale.company_name }}</td>
								</tr>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			getSaleDetails() {
				let filter = {
					companyId: this.selectedCompany == null || this.selectedCompany.Company_SlNo == '' ? '' : this.selectedCompany.Company_SlNo,
			...
			async print() {
				...
				let companyText = '';
				if (this.selectedCompany != null && this.selectedCompany.Company_SlNo != '' && this.searchType == 'company') {
