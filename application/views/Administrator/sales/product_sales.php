                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-5">
                            <form v-on:submit.prevent="addToCart">
                                <div class="form-group">
                                    <label class="col-xs-3 control-label no-padding-right"> Company </label>
                                    <div class="col-xs-9">
                                        <v-select v-bind:options="companies" v-model="selectedCompany"
                                            label="Company_Name" v-on:input="companyOnChange"></v-select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-3" style="display: none;">
                                        <input type="number" min="0" step="any" id="productDiscountPer"
                                            class="form-control" v-model="productDiscountPer"
                                            v-on:input="productTotal" />
                                    </div>
                                    <label style="display: none;"
                                        class="col-xs-1 control-label no-padding-right">%</label>
                                    <div class="col-xs-9">
                                        <input type="number" min="0" step="any" id="productDiscount"
                                            class="form-control" v-model="selectedProduct.productDiscount"
                                            v-on:input="productTotal" />
                                    </div>
                                </div>
                ...
                <table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
                    <thead>
                        <tr class="">
                            ...
                            <th style="width:10%; color:#000;">Company</th>
                            <th style="width:10%; color:#000;">Category</th>
                            <th style="width:5%; color:#000;">Qty</th>
                            <th style="width:10%; color:#000;">Rate</th>
                            <th style="width:10%; color:#000;">Total</th>
                            <th style="width:10%; color:#000;">Discount</th>
                    ...
                    <tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
                        ...
                        <tr>
                            ...
                            <td colspan="2" style="padding-top: 15px;font-size:18px;">
                                {{ cart.reduce((prev,curr) => {return prev + parseFloat(curr.productDiscount)}, 0).toFixed(2) }}
                            </td>
                                ...
                                <table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-group">
                                                <label
                                                    class="col-xs-12 control-label no-padding-right">Collection</label>
                                                <div class="col-xs-12">
...
new Vue({
    ...
    methods: {
        ...
        productTotal() {
            ...
            let total = parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct
                .Product_SellingPrice);
            let discount = parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct
                .productDiscount);

            // if (event.target.id == 'productDiscountPer') {
            // 	this.selectedProduct.productDiscount = ((parseFloat(total) * parseFloat(this.productDiscountPer)) / 100).toFixed(2);
            // } else {
            // 	this.productDiscountPer = (parseFloat(this.selectedProduct.productDiscount) / parseFloat(total) * 100).toFixed(2);
            // }

            this.selectedProduct.total = (parseFloat(total) - parseFloat(discount)).toFixed(2);
        ...
        async getCustomerDue() {
            if (this.selectedCustomer.is_supplier == 1) {
        ...
        companyOnChange() {
            if (this.selectedCompany.Company_SlNo == '') {
                return;
            }
            axios.post('/get_products', {
                companyId: this.selectedCompany.Company_SlNo,
                isService: this.sales.isService
            }).then(res => {
                // this.products = res.data;
        ...
        async productOnChange() {
            if ((this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0) && this
                ...
                // this.productDiscountPer = 0;
                // this.selectedProduct.productDiscount = 0;
                this.productTotal();
            }
        ...
        addToCart() {
            let product = {
                ...
                companyName: this.selectedProduct.Company_Name,
                name: this.selectedProduct.Product_Name,
                salesRate: this.selectedProduct.Product_SellingPrice,
                vat: this.selectedProduct.vat,
                productDiscount: (parseFloat(this.selectedProduct.productDiscount) * parseFloat(this
                    .selectedProduct.quantity)).toFixed(2),
                quantity: this.selectedProduct.quantity,
                total: (parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct
                    .Product_SellingPrice)).toFixed(2),
                ftotal: ((parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct
                    .Product_SellingPrice)) - (parseFloat(this.selectedProduct.productDiscount) *
                    parseFloat(this.selectedProduct.quantity))).toFixed(2),
        ...
        calculateTotal() {
            this.sales.subTotal = this.cart.reduce((prev, curr) => {
                return prev + parseFloat(curr.ftotal)
            }, 0).toFixed(2);

            this.sales.vat = this.cart.reduce((prev, curr) => {
                return +prev + +(curr.ftotal * (curr.vat / 100))
            }, 0);
            ...
            await axios.post('/get_sales', {
                ...
                r.saleDetails.forEach(product => {
                    let cartProduct = {
                        ...
                        ftotal: product.SaleDetails_TotalAmount - product
                            .Discount_amount,
