						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-xs-9">
										<v-select v-bind:options="companies" v-model="selectedCompany" label="Company_Name" v-on:input="companyOnChange"></v-select>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-xs-3" style="display: none;">
										<input type="number" min="0" step="any" id="productDiscountPer" class="form-control" v-model="productDiscountPer" v-on:input="productTotal" />
									</div>
									<label style="display: none;" class="col-xs-1 control-label no-padding-right">%</label>
									<div class="col-xs-9">
										<input type="number" min="0" step="any" id="productDiscount" class="form-control" v-model="selectedProduct.productDiscount" v-on:input="productTotal" />
									</div>
								</div>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-xs-12 control-label no-padding-right">Collection</label>
												<div class="col-xs-12">
	...
	new Vue({
		...
		methods: {
			...
			productTotal() {
				...
				let discount = parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct.productDiscount);
			...
			addToCart() {
				let product = {
					...
					productDiscount: (parseFloat(this.selectedProduct.productDiscount) * parseFloat(this.selectedProduct.quantity)).toFixed(2),
					quantity: this.selectedProduct.quantity,
					total: (parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct.Product_SellingPrice)).toFixed(2),
					ftotal: ((parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct.Product_SellingPrice)) - (parseFloat(this.selectedProduct.productDiscount) * parseFloat(this.selectedProduct.quantity))).toFixed(2),
				...
				await axios.post('/get_sales', {
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							ftotal: product.SaleDetails_TotalAmount - product.Discount_amount,
